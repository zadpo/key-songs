...
export function TrackDashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        tracksSnapshot.forEach((doc) => {
          ...
          fetchedTracks.push({ ...track, id: doc.id, date: new Date(track.date) });
          if (track.title) artistsSet.add(track.title.split(" - ")[0]);
        });
        const totalArtists = artistsSet.size;

        const recentUploadsQuery = query(tracksCollection, orderBy("date", "desc"), limit(5));
        ...
        const recentActivityList = recentUploadsSnapshot.docs.map((doc) => {
          const track = doc.data() as Track;
          return `New track "${track.title}" added`;
        });
        ...
        setChartData([
          { name: "Jan", total: monthCounts[0] },
          { name: "Feb", total: monthCounts[1] },
          { name: "Mar", total: monthCounts[2] },
          { name: "Apr", total: monthCounts[3] },
  ...
  return (
    <ProtectedRoute>
      <div className="container mx-auto py-6 flex items-center justify-center flex-col">
        <h1 className="text-3xl font-bold mb-6">Track Manager Dashboard</h1>
        <div className="grid aspect-video w-full max-w-4xl justify-center text-foreground md:grid-cols-2 [&>div]:relative [&>div]:flex [&>div]:h-[137px] [&>div]:w-[224px] [&>div]:items-center [&>div]:justify-center [&>div]:p-4">
          <div>
            <div className="absolute left-[-48px] top-[45px] z-10 text-sm font-medium">Tracks</div>
            <svg
            ...
            <TooltipDemo
              label="Track Stats"
              payload={[
                { name: "Total Tracks", value: stats.totalTracks, fill: "hsl(var(--chart-1))" },
                { name: "Recent Uploads", value: stats.recentUploads, fill: "hsl(var(--chart-2))" },
              ]}
              className="w-[8rem]"
            />
          </div>
          <div className="items-end">
            <div className="absolute left-[122px] top-[0px] z-10 text-sm font-medium">Artists</div>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="35"
              height="42"
            ...
            <TooltipDemo
              label="Artist Stats"
              hideLabel
              payload={[{ name: "Total Artists", value: stats.totalArtists, fill: "hsl(var(--chart-3))" }]}
              indicator="dashed"
              className="w-[8rem]"
            />
          </div>
